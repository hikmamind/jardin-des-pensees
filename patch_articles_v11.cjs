...
// 1. Fix the slug bug. We need to add showArticleNotFound and update openArticleReader.
const openReaderStart = articlesJS.indexOf('function openArticleReader(file) {');
if (openReaderStart !== -1) {
  // First, inject showArticleNotFound right above openArticleReader
  const notFoundFunc = `
...
`;
  articlesJS = articlesJS.substring(0, openReaderStart) + notFoundFunc + articlesJS.substring(openReaderStart);
  
  // Then fix the matching logic in openArticleReader
  const badMatch = `const article = articles.find(a => a.file === file) || articles[0];
  if (!article) return;`;
  const goodMatch = `const article = articles.find(a => a.file === file || a.id === file || a.file === file + '.html');
  if (!article) {
    showArticleNotFound();
    return;
  }
...
`;
  articlesJS = articlesJS.replace(badMatch, goodMatch);
}

// 2. Patch the body rendering inside openArticleReader
const openReaderMatch = "function openArticleReader";
const indexOfOpenReader = articlesJS.indexOf(openReaderMatch);
const targetBodyElIf = articlesJS.indexOf("if (bodyEl) {", indexOfOpenReader);
const endOfBodyElIf = articlesJS.indexOf("// Update browser URL query param", targetBodyElIf);

if (targetBodyElIf !== -1 && endOfBodyElIf !== -1) {
...
`;
  articlesJS = articlesJS.substring(0, targetBodyElIf) + newBodyLogic + articlesJS.substring(endOfBodyElIf);
}
...
console.log('Successfully patched articles_v11.js');
