...
function displayNewWisdom(isInitial = false) {
  ...
  // In translated mode, we display the translated text
  // The secondary translation line displays the original Arabic if the active language is not Arabic,
  // or a French translation if the active language is Arabic.
  let primaryText = `"${quote.text}"`;
  let secondaryText = "";

  if (currentLang === 'ar') {
    // Find the corresponding french quote text to show as secondary translation
    // If not found, fall back to french in translation map or Seneca placeholder
    ...
    // Show the Arabic text as translation
...
function populateProfile() {
  ...
  // Stats labels are handled via data-i18n. We re-animate counts only on initial load.
  // To avoid constant animations on language toggling, we check if they are already animated or not
...
function populateAdvices() {
  ...
  container.innerHTML = advices.map(advice => {
    ...
    // Translated thinker introductory prefix
      ...
      <div class="video-card" data-youtube-id="${video.youtubeId}">
        <div class="video-thumbnail-container">
          <div class="video-thumbnail-placeholder">
            <!-- Zen Lotus SVG Icon representing wisdom & garden -->
...
function initNavigation() {
  ...
  window.addEventListener('scroll', () => {
    ...
    sections.forEach(sec => {
      ...
      // scrollSpy boundary trigger (navbar height offset)
...
function setupContactForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    // Basic Validation (using translated warnings)
    ...
    setTimeout(() => {
      ...
      // Reset button contents from dynamic translation key
