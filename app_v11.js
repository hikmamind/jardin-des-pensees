...
function populateArticles(filterCategory = 'all') {
  ...
  container.innerHTML = homeArticles.map(art => {
    ...
    return `
      const targetUrl = art.file ? `./articles/${art.file}` : `./files/${art.file || 'stop-overthinking.html'}`;
      return `
        <div class="article-card" data-category="${art.category}" data-file="${targetUrl}" style="cursor: pointer;">
